...
class Synchroniser {
	...
	void update(int frameCount) { // where there are 24 frames in a beat
		...
		int new16th = frameCount /6 ;
		int new8th = new16th/2;
		int newBeat = new8th/2;
		int newBar = newBeat/4;
		
		//float msPerBar = 60000.0f / tempo * 4;
		barPulse = 1.0f - ((float)(frameCount%96) / 96.0f);
		//float msPerBeat = msPerBar/4;
		beatPulse = 1.0f - ((float)(frameCount%24) / 24.0f);
		//float msPer8th = msPerBeat/2;
		eighthPulse = ((float)(frameCount%12) / 12.0f);
		
		//float msPer16th = msPer8th/2;
		sixteenthPulse = ((float)(frameCount%6) / 6.0f);
		...
		currentBarFloat = (float)frameCount / 96.0f; //(position-startPosition)/((60000.0f/tempo) * 4);
		
		currentPosition = (float)frameCount / 24.0f * 60.0f/ tempo ;
		
		// bpm
		// mpb = 1/bpm
		// seconds per beat = mpb*60
